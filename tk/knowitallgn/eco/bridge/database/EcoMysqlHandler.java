...
public class EcoMysqlHandler {
  ...
  public String[] getData(Player player) {
    ...
    Connection conn = this.eco.getMysqlSetup().getConnection();
    if (conn != null)
      try {
        String sql = "SELECT * FROM `" + this.eco.getConfigHandler().getString("database.mysql.dataTableName") + "` WHERE `player_uuid` = ? LIMIT 1";
        ...
        if (result.next()) {
          String[] data = { result.getString("money"), result.getString("sync_complete"), result.getString("last_seen") };
          return data;
        } 
  ...
  public void setSyncStatus(Player player, String syncStatus) {
    ...
    if (conn != null)
      try {
        String data = "UPDATE `" + this.eco.getConfigHandler().getString("database.mysql.dataTableName") + "` " + "SET `sync_complete` = ?" + ", `last_seen` = ?" + " WHERE `player_uuid` = ?";
  ...
  public void setData(Player player, Double money, String syncComplete) {
    ...
    if (conn != null)
      try {
        String data = "UPDATE `" + this.eco.getConfigHandler().getString("database.mysql.dataTableName") + "` " + "SET `player_name` = ?" + ", `money` = ?" + ", `sync_complete` = ?" + ", `last_seen` = ?" + " WHERE `player_uuid` = ?";
        preparedUpdateStatement = conn.prepareStatement(data);
        preparedUpdateStatement.setString(1, player.getName());
        preparedUpdateStatement.setDouble(2, money.doubleValue());
  ...
  public void createAccount(Player player) {
    ...
    if (conn != null)
      try {
        String sql = "INSERT INTO `" + this.eco.getConfigHandler().getString("database.mysql.dataTableName") + "`(`player_uuid`, `player_name`, `money`, `last_seen`, `sync_complete`) " + "VALUES(?, ?, ?, ?, ?)";
  ...
  public boolean hasAccount(Player player) {
    ...
    if (conn != null)
      try {
        String sql = "SELECT `player_uuid` FROM `" + this.eco.getConfigHandler().getString("database.mysql.dataTableName") + "` WHERE `player_uuid` = ? LIMIT 1";
        preparedUpdateStatement = conn.prepareStatement(sql);
        preparedUpdateStatement.setString(1, player.getUniqueId().toString());
        result = preparedUpdateStatement.executeQuery();
        if (result.next())
